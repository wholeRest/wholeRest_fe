...
})(({ theme, isSelected, isHovered, isStartDate, isEndDate, color, emoji  }) => ({
  borderRadius: 0,
  ...(isSelected && {
...
function Day(props) {
  ...
  return (
    <CustomPickersDay
      ...
      emoji={range?.emoji} // 추가: 이모지 속성 전달
...
export function Calendar() {
  ...
  const [popupOpen_emoji, setPopupOpen_emoji] = React.useState(false); // 이모지 선택 팝업 눌렀나
  const [selectedEmoji, setSelectedEmoji] = React.useState(''); // 선택된 이모지
  const [emojiDate, setEmojiDate] = React.useState(formatDate(new Date())); // 이모지를 표시할 날짜
    ...
    if (newStartDate.isValid() && newEndDate.isValid() && newStartDate.isBefore(newEndDate)) {
      if (selectedRange) {
        ...
        setRanges(ranges.map((range) =>
          range === selectedRange
            ? { ...range, startDate: newStartDate, endDate: newEndDate, color, emoji: selectedEmoji }
            : range
        ));
        setSelectedRange(null);
      } else {
        // 새로운 범위 추가
        setRanges([...ranges, { startDate: newStartDate, endDate: newEndDate, color, emoji: selectedEmoji }]);
      }
      setStartDateInput('');
      setEndDateInput('');
  ...
  const handleDayClick = (day) => {
    ...
    if (range) {
      ...
      setSelectedEmoji(range.emoji || ''); // 선택된 이모지 업데이트
  ...
  const handleEmojiSelect = (emoji) => {
    setSelectedEmoji(emoji); // 선택된 이모지 상태 업데이트
    setPopupOpen_emoji(false); // 이모지 선택 팝업 닫기
  };
...
{/*
        ...
        <div className={`calendar_rangePopupDiv ${popupOpen_emoji ? 'checked' : ''}`}>
          ...
          <div className='calendarPop_dateSelector'>
          <TextField
            label="Select Date"
            type="date"
            value={emojiDate}
            onChange={(e) => setEmojiDate(e.target.value)}
            InputLabelProps={{ shrink: true }}
          />
          </div>
              ...
              <div className='calendarPop_emojis'>
                ...
                <div className='todayEmojiDiv' onClick={() => handleEmojiSelect('😀')}>
                  <svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" fill="none">
                    ...
                    <path d="M12 12H14V14H12V12ZM16 12H18V14H16V12ZM13 16H17V18H13V16Z" fill="#000000"/>
                  </svg>
                </div>
                ...
                <div className='todayEmoji_down'>
                  ...
                  <div className='todayEmojiDiv'>
                  <svg 
                  style={{  border: selectedEmoji === 'neutralIcon' ? '2px solid #FFEFA6' : 'none' }}
                  onClick={() => {
                    selectedEmoji === 'neutralIcon' ? setSelectedEmoji('') : setSelectedEmoji('neutralIcon');
                  ...
                  <div className='todayEmojiDiv'>
                  <svg 
                  style={{  border: selectedEmoji === 'cryIcon' ? '2px solid #FFEFA6' : 'none' }}
                  onClick={() => {
                    selectedEmoji === 'cryIcon' ? setSelectedEmoji('') : setSelectedEmoji('cryIcon');
        ...
        <div className='calendarPop_Btn'>
          <Button variant="contained" onClick={() => {
            setRanges([
              ...ranges,
              { startDate: dayjs(emojiDate), endDate: dayjs(emojiDate), color, emoji: selectedEmoji }
            ]);
            setPopupOpen_emoji(false);
          }}>
            이모지 등록하기
          </Button>
        </div>
